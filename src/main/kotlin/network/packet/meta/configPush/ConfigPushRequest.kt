...
class ConfigPushRequest(val client: QQClient, packet: TransportPacket.Response.Buffered) :
    TransportPacket.Response.Simple(packet) {

    lateinit var action: ConfigPushAction
        private set

    override fun readBody(input: ByteBuf) {
        RequestPacket(input.readJceStruct()).use { packet ->
            RequestDataV2(packet.buffer.readJceStruct()).use {
                action =
                    ConfigPushAction(it["PushReq"]!!["ConfigPush.PushReq"]!!.unwrapUniRequestData().readJceStruct())
            }
        }
    }
