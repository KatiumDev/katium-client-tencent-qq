...
class PullFriendListResponse(val client: QQClient, packet: TransportPacket.Response.Buffered) :
    TransportPacket.Response.Simple(packet) {

    lateinit var response: PullFriendListResponseData
        private set
    ...
    override fun readBody(input: ByteBuf) {
        RequestPacket(input.readJceStruct()).use {
            RequestDataV3(it.buffer.readJceStruct()).use { data ->
                response = PullFriendListResponseData(data["FLRESP"]!!.unwrapUniRequestData().readJceStruct())
            }
        }
    }
