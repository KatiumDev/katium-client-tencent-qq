...
object QQServiceMessageEncoder : MessageEncoder<QQServiceMessage> {
    ...
    override suspend fun encode(
        client: QQClient, context: QQChat, message: QQServiceMessage, isStandalone: Boolean
    ...
    override suspend fun createGeneralFlags(
        client: QQClient, context: QQChat, message: QQServiceMessage
    ) = if (message.type == QQServiceMessage.Type.LONG_MESSAGE) arrayOf(
        PbMessageElements.GeneralFlags.newBuilder().setLongTextFlag(1).setLongTextResourceID(message.resourceID)
            .setPbReserve(ByteString.fromHex("7800F80100C80200")).build()
    )
    else arrayOf(
