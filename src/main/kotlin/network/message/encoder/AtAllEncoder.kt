...
object AtAllEncoder : MessageEncoder<AtAll> {
    ...
    override suspend fun encode(client: QQClient, context: QQChat, message: AtAll, isStandalone: Boolean) =
        message.run {
            arrayOf(
                PbMessageElements.Element.newBuilder().setText(
                    PbMessageElements.Text.newBuilder().setString(DISPLAY).setAttribute6Buf(
                        ByteString.copyFrom(
                ...
                PbMessageElements.Element.newBuilder().setText(PbMessageElements.Text.newBuilder().setString(" "))
                    .build()
            )
        }
